    ...
    <div className="crm-auth-portal flex min-h-screen items-center justify-center px-4 py-6 sm:py-8">
      <div className="w-full max-w-sm">
        <div className="mb-5 flex flex-col items-center gap-2">
          <div className="flex size-10 items-center justify-center rounded-md bg-[#1e40af] text-white shadow-sm"><Megaphone className="size-5" /></div>
          <div className="text-center"><h1 className="text-xl font-bold text-[#1e40af]">AdFlow Pro</h1><p className="mt-0.5 text-xs text-slate-500">Facebook Ads agency operations, in one place</p></div>
        </div>
        <div className="crm-auth-card overflow-hidden rounded-md border bg-white shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 p-4 sm:p-5">
            ...
            <div><label className="crm-label" htmlFor="password">Password</label><div className="relative"><LockKeyhole className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-600" /><input id="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="crm-input pl-9 pr-10" type={showPassword ? "text" : "password"} autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={mode === "register" ? 12 : 1} /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-500 hover:text-slate-700" aria-label={showPassword ? "Hide password" : "Show password"} title={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div>{mode === "register" && <p className="mt-1.5 text-xs text-slate-500">Use 12+ characters with uppercase, lowercase, number, and special character.</p>}</div>
            <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-md bg-[#1e40af] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1e3a8a] disabled:opacity-60">{loading && <Loader2 className="size-4 animate-spin" />}{loading ? "Authenticating..." : mode === "login" ? "Sign In" : "Create Workspace"}</button>
          </form>
        </div>
      </div>
    </div>
