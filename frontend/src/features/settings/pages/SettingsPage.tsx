...
export function SettingsPage({ agencyId, onWorkspaceRefresh }: { agencyId: string; onWorkspaceRefresh: () => Promise<boolean> }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (accessToken.trim() || overview?.connection.isConnected) await saveFacebookSettings(agencyId, { accessToken: accessToken.trim() || undefined, defaultAdAccountId: account || undefined });
