...
export type CreateInvoicePayload = { client: string; adRequest: string; pageName: string; objective: string; budget: { amount: number; type: "daily" | "lifetime"; currency: string }; durationDays: number; rate: number; amount: number; dueDate: string };
export type UpdateInvoicePayload = { client?: string; adRequest?: string; amount?: number; currency?: "BDT" | "USD" | "INR"; status?: InvoiceStatus; dueDate?: string };
